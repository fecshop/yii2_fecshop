...
<body>
    <div class="header">
        <div>
            <img style="height:50px;" src="<?= Yii::$service->image->getFecmallLogoImgUrl() ?>" />
        ...
        <?php endif; ?>
        <h2><?php 
            $mes = nl2br($handler->htmlEncode($exception->getMessage())) ;
            echo $mes;
            if ($mes == 'this domain is not config in store service, you must config it in admin store config') {
                echo '<br/><br/>';
                echo '出现该报错，是因为您在fecmall安装过程中填写的域名和nginx配置的域名不一致，或者填写域名格式错误导致的，您可以点击这里参看帖子进行解决：<br/>
                <a target="_blank" href="http://www.fecmall.com/topic/4582">Fecmall安装报错【已解决】：this domain is not config in store service, you must config it in admin store config</a>';
            }
        ?></h2>
    ...
    <div class="footer">
        ...
        <p><a href="http://www.fecmall.com/">Fecmall Framework</a>/<?= Yii::$service->helper->getVersion() ?></p>
    </div>
